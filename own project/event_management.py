...
def download_pdf():
    ...
    try:
        ...
        print(f"Database error in download_pdf: {str(e)}")
...
def show_participant():
    try:
        ...
        print(f"Fetched {len(fetched_data)} records: {fetched_data}")
        ...
        print(f"Database error in show_participant: {str(e)}")
        messagebox.showerror('Error', f'Database error: {str(e)}', icon='error')
...
def delete_participant():
    ...
    try:
        ...
        con.rollback()
        print(f"Database error in delete_participant: {str(e)}")
        messagebox.showerror('Error', f'Database error: {str(e)}', icon='error')
...
def search_participant():
    def search_data():
        ...
        try:
            ...
            mycursor.execute(query, params)
            fetched_data = mycursor.fetchall()
            print(f"Search fetched {len(fetched_data)} records: {fetched_data}")
            participantTable.delete(*participantTable.get_children())
            for i, data in enumerate(fetched_data):
                tag = 'evenrow' if i % 2 == 0 else 'oddrow'
                values = [data[col] for col in ['register_no', 'name', 'department', 'event_name', 'from_date', 'to_date', 'event_location', 'mobile_no', 'email', 'address', 'gender', 'date', 'time']]
            ...
            print(f"Database error in search_data: {str(e)}")
            messagebox.showerror('Error', f'Database error: {str(e)}', parent=search_window, icon='error')
        except Exception as e:
            print(f"General error in search_data: {str(e)}")
...
def add_participant():
    def add_data():
        ...
        try:
            ...
            if not register_no:
                messagebox.showerror('Error', 'Register Number is required!', parent=add_window, icon='error')
                return
            ...
            # Check for duplicate register_no
            query = 'SELECT COUNT(*) FROM participant WHERE register_no=%s'
            ...
            print(f"Inserting data: register_no={register_no}, name={nameEntry.get()}, department={departmentEntry.get()}, event_name={event_name_upper}, from_date={from_date}, to_date={to_date}, event_location={eventlocationEntry.get()}, mobile_no={mobile_value}, email={email_value}, address={addressEntry.get()}, gender={genderEntry.get()}, pdf_file={'Yes' if pdf_data else 'No'}, date={current_date}, time={current_time}")
            mycursor.execute(query, (
                register_no, nameEntry.get(), departmentEntry.get(), event_name_upper,
...
def connect_database():
    def connect():
        ...
        try:
            ...
            print("Database 'participant' ensured")
            ...
            if not table_exists:
                print("Creating new participant table")
                mycursor.execute(''' 
                ...
                print("Table 'participant' already exists")
            ...
            print(f"Database Error in connect: {str(e)}")
...
addparticipantButton = ttk.Button(leftFrame, text='Add Participant', style='Yellow.TButton', width=20, state=DISABLED, command=add_participant)
