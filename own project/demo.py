...
def set_frame_background(frame, image_path, relwidth, relheight, window):
    """
    Set a background image for a specific frame.
    :param frame: The frame to apply the background to
    :param image_path: Path to the image file
    :param relwidth: Relative width of the frame
    :param relheight: Relative height of the frame
    :param window: The parent window to get screen dimensions
    """
    try:
        if not os.path.exists(image_path):
            raise FileNotFoundError(f"Image file {image_path} not found")
        ...
        frame.configure(bg='#15F30D')  # Fallback to original background color
...
def connect_database():
    ...
    # Set window background to college_background.jpg
    ...
    # Set frame background to database_background.jpg
    set_frame_background(form_frame, 'database_background.jpg', relwidth=0.8, relheight=0.8, window=connect_window)
